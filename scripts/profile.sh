# 현재 쉬고 있는 profile과 port 체크
# # start, stop, switch, health 스크립트 파일에서 공용으로 사용

#!/usr/bin/env bash

# 쉬고 있는 profile 찾기 : real1이 사용 중이면 real2가 쉬고, real2가 사용 중이면 real이 쉼
# 쉬고 있는 profile을 새로 배포한 스프링 부트 프로젝트와 연결
function find_idle_profile() {
    ...
    if [ ${RESPONSE_CODE} -ge 400 ] # HttpStatus Code가 400 이상이면 즉, 오류가 발생한다면 real2를 현재 profile로 사용
    then
